...
ggplot(stateData, aes(x=long, y=lat))+ geom_polygon(aes(group=group, fill=avgYoungVoterPercent), color="white") + ggtitle("Average Percent Congressional District's Voters 18-29 by State") + scale_fill_distiller(name="Percent Composition", palette = "YlOrRd")
```
...
voting.rate_map <- map_base %>%
  inner_join(map_data %>% 
               ...
               mutate(DISTRICT = DISTRICT - 1),  #DELETE THIS LINE IF THE MAPS ARE NOT SHOWING UP
             
             by=c('State.name'='STATENAME','Congressional.district'='DISTRICT'))
