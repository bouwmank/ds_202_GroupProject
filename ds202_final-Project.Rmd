...
ggplot(eduPol, aes(x=PercentageEdu, y=Voting.rate.estimate)) + geom_point() + geom_smooth(method='lm') + facet_wrap(~Education)
...
table02a <- table02b %>%
  select(stateName, congressDist, Men.estimate, Men.percent.total, Women.estimate, Women.percent.total) %>%
  inner_join(table02a, by=c("stateName", "congressDist"))
head(table02a)
```
